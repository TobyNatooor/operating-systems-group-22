...
int simple_macro_test() {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (GET_NEXT(p) != addr[i])
      return 1 + i * 10; // Next pointer damaged
    if (GET_FREE(p) != 1)
      return 2 + i * 10; // Free flag not set

    SET_NEXT(p, NULL);
    if (GET_FREE(p) != 1)
      return 3 + i * 10; // Free flag damaged

    SET_NEXT(p, addr[i]);
    SET_FREE(p, 0);

    if (GET_FREE(p) != 0)
      return 4 + i * 10; // Free flag not cleared
    if (GET_NEXT(p) != addr[i])
      return 5 + i * 10; // Next pointer damaged
    ...
    SET_NEXT(p, (void *)((uintptr_t)p + sizeof(BlockHeader) - 0x100));
    if (SIZE(p) != 0 && SIZE(p) < 0x800000000000000)
      return 7 + i * 10;
  }
  return ret;
}
...
static void print_block(BlockHeader *p) {
  printf("Block at 0x%08lx next = 0x%08lx, free = %d, size: %li\n", (uintptr_t)p,
         (uintptr_t)GET_NEXT(p), GET_FREE(p), SIZE(p));
}
